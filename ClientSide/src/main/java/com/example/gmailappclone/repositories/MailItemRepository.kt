...
class MailItemRepository(private val mailLayoutItemDao: MailLayoutItemDao) {
    ...
    fun getMailsByLabel(givenLabel : String) : Flow<List<MailLayoutItem>> = mailLayoutItemDao.getMailsByLabel(givenLabel)

    suspend fun deleteAllMails() = mailLayoutItemDao.deleteAllMails()
